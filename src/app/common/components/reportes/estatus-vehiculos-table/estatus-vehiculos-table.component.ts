...
@Component({
  ...
  templateUrl: './estatus-vehiculos-table.component.html',
  styleUrls: ['./estatus-vehiculos-table.component.scss'],
})
export class EstatusVehiculosTableComponent implements OnInit {
  ...
  loadVehiculosTable(_data?: VehiculosI[]) {
    ...
    if (_data) {
      ...
      this.reportesServ.getEstatusVehiculosReport().subscribe(response => {
        if (response.ok === true) {
          this.spinner = false;
